...
static timestamp_t get_time_and_update_history(BalanceInfo *info) {
...
static void child_process(IpcData *ipc, balance_t initial_balance) {
    ...
    while (!stop_received) {
        ...
        if (receive_any(ipc, &recv_msg) == 0) {
            time = get_time_and_update_history(balance_info);
            
            if (recv_msg.s_header.s_type == TRANSFER) {
                TransferOrder *order = (TransferOrder *)recv_msg.s_payload;
                
                if (ipc->local_id == order->s_src) {
                    update_balance_history_to(balance_info, time);
                    balance_info->balance -= order->s_amount;
                    update_balance_history(balance_info, time);
                    
                    fprintf(ipc->events_log, log_transfer_out_fmt, time, ipc->local_id, order->s_amount, order->s_dst);
                    fflush(ipc->events_log);
                    printf(log_transfer_out_fmt, time, ipc->local_id, order->s_amount, order->s_dst);
                    fflush(stdout);
                    
                    recv_msg.s_header.s_local_time = time;
                    ...
                    fprintf(ipc->events_log, log_transfer_in_fmt, time, ipc->local_id, order->s_amount, order->s_src);
                    fflush(ipc->events_log);
                    printf(log_transfer_in_fmt, time, ipc->local_id, order->s_amount, order->s_src);
                    fflush(stdout);
    ...
    history_msg.s_header.s_local_time = time;

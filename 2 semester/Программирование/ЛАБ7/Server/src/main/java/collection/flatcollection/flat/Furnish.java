...
public enum Furnish { // обстановка
    ...
    public static Furnish getFurnishByName(String name) {
        if(name.equals("DESIGNER")) return DESIGNER;
        if(name.equals("NONE")) return NONE;
        if(name.equals("FINE")) return FINE;
        if(name.equals("BAD")) return BAD;
        if(name.equals("LITTLE")) return LITTLE;
        return null;
    }

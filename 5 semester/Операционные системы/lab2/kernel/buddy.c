...
// Invert bit at position in array
void pair_toggle(char *array, int index) {
  char b = array[index / 16];
  char m = (1 << ((index % 16) / 2));
  array[index / 16] = (b ^ m);
}

int pair_isset(char *array, int index) {
  char b = array[index / 16];
  char m = (1 << ((index % 16) / 2));
  return (b & m) == m;
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  pair_toggle(bd_sizes[k].alloc, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    pair_toggle(bd_sizes[k - 1].alloc, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    pair_toggle(bd_sizes[k].alloc, bi);           // free p at size k
    if (pair_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      pair_toggle(bd_sizes[k].alloc, bi);
    }
  }
}
...
int bd_initfree_pair_right(int k, int bi) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (pair_isset(bd_sizes[k].alloc, bi)) {
